...
const connection = mysql.createConnection({
    host: '172.19.0.2', // Хост базы данных
...
async function checkUserTariff(userId, advertId, currentTime) {
    try {
        ...
        if (results.length > 0 && results[0].status === 'old') {
            // У пользователя есть активный тариф
            await processPayment(userId, advertId, results[0].price_day_one_advert, currentTime);
        }
    } catch (error) {
